...
<template>
  <div class="h-full w-full flex gap-4 px-4 md:px-7 py-5 relative overflow-hidden">
    ...
    <!-- ════════════════════════════════════════════════════════════
         ...
         ════════════════════════════════════════════════════════════ -->
    <AppCard class="relative z-10 shrink-0 w-[400px] !p-0 flex flex-col overflow-hidden">
      <!-- Banner pastel arriba — solo nubes en esquinas, sin sparkles invasivos -->
      <div class="relative h-[190px] bg-sky-soft shrink-0 overflow-hidden">
        <HibiCloud :size="100" float :duration="6" class="absolute top-4 left-5 text-card opacity-85" aria-hidden="true" />
        <HibiCloud :size="64"  float :duration="8" :delay="0.7" class="absolute bottom-2 right-6 text-card opacity-65" aria-hidden="true" />
      </div>

      <!-- Avatar XXL superpuesto -->
      <div class="relative -mt-28 px-6 shrink-0 flex flex-col items-center">
        <div class="relative">
          <div class="rounded-full p-1.5 bg-card">
            <AppAvatar :name="profileName" :src="avatarSrc ?? undefined" :size="240" />
          </div>
          ...
          <button type="button"
            ...
            style="box-shadow: inset 0 0 0 3px var(--color-sky-soft, #d9edfa);"
          ...
          <button v-if="avatarSrc" type="button"
            ...
            style="box-shadow: inset 0 0 0 2.5px var(--color-pink-soft, #fbe6ec);"
        ...
        <input v-model="profileName" type="text" placeholder="Tu nombre"
          class="mt-4 text-[22px] font-extrabold text-fg leading-tight bg-transparent outline-none w-full text-center focus:bg-muted px-2 py-1 rounded-[10px] transition-[background-color]" />
    ...
    <!-- ════════════════════════════════════════════════════════════
         ...
         ════════════════════════════════════════════════════════════ -->
    <div class="relative z-10 flex-1 min-w-0 min-h-0 flex flex-col gap-4">
      ...
      <section class="shrink-0">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <AppCard class="!p-5 flex items-center justify-between gap-4">
            ...
            <AppSegmented :model-value="locale" :options="langOptions" @update:model-value="onLang" />
          </AppCard>
      ...
      <!-- Bloque notificaciones — ocupa el resto del alto -->
      <section class="flex-1 min-h-0 flex flex-col">
        ...
        <AppCard class="flex-1 min-h-0 !p-5 flex flex-col">
          ...
          <div class="flex-1 min-h-0 grid grid-cols-1 md:grid-cols-3 gap-3">
