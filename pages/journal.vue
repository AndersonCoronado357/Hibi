...
useHead({ title: t('journal.head.title') })
...
const moods = computed<Mood[]>(() => [
  { v: 1, icon: markRaw(Angry), label: t('journal.moods.1'), color: 'text-pink-deep',  bg: 'bg-pink-soft', hex: '#db8aa3' },
  { v: 2, icon: markRaw(Frown), label: t('journal.moods.2'), color: 'text-[#c5733f]',  bg: 'bg-peach',     hex: '#c5733f' },
  { v: 3, icon: markRaw(Meh),   label: t('journal.moods.3'), color: 'text-[#bf8f2e]',  bg: 'bg-cream',     hex: '#bf8f2e' },
  { v: 4, icon: markRaw(Smile), label: t('journal.moods.4'), color: 'text-[#34936a]',  bg: 'bg-mint',      hex: '#34936a' },
  { v: 5, icon: markRaw(Laugh), label: t('journal.moods.5'), color: 'text-sky-deep',   bg: 'bg-sky-soft',  hex: '#5aa6d2' },
])

const emptyIcon = markRaw(BookHeart)

// Iniciales de los días de la semana (L..D), traducibles.
const weekdayLabels = computed(() => [
  t('journal.weekdays.0'), t('journal.weekdays.1'), t('journal.weekdays.2'), t('journal.weekdays.3'),
  t('journal.weekdays.4'), t('journal.weekdays.5'), t('journal.weekdays.6'),
])
...
const PROMPTS = computed(() => [
  t('journal.prompts.0'), t('journal.prompts.1'), t('journal.prompts.2'),
  t('journal.prompts.3'), t('journal.prompts.4'),
])
const promptIdx = ref(0)
const prompt = computed(() => PROMPTS.value[promptIdx.value] ?? PROMPTS.value[0])
function newPrompt() {
  let next = promptIdx.value; let safety = 0
  while (next === promptIdx.value && safety++ < 10) next = Math.floor(Math.random() * PROMPTS.value.length)
  promptIdx.value = next
}
...
const moodDist = computed(() => {
  ...
  return moods.value.map(m => ({ ...m, count: entries.value.filter(e => e.mood === m.v).length, pct: Math.round((entries.value.filter(e => e.mood === m.v).length / total) * 100) }))
})
...
<template>
  <div class="h-full flex flex-col gap-2 md:gap-3 px-3 md:px-7 py-3 md:py-5 relative overflow-hidden">
    ...
    <div class="relative z-10">
      <PageHero :icon="BookHeart" tone="pink" :title="t('journal.title')" :subtitle="t('journal.subtitle', { count: entries.length, avg: moodAverage.toFixed(1) })">
        <template #actions>
          <AppSegmented v-model="view" :options="[
            { value: 'editor', icon: NotebookPen, ariaLabel: t('journal.views.editor') },
            { value: 'stats',  icon: BarChart3,   ariaLabel: t('journal.views.stats') },
          ]" />
          <AppButton variant="primary" size="sm" class="ml-auto" @click="selectedDate = new Date(); view = 'editor'"><template #icon><Plus class="size-[16px]" :stroke-width="2.3" /></template>{{ t('journal.today') }}</AppButton>
        </template>
      </PageHero>
    </div>
    ...
    <div v-if="view === 'editor'" class="relative flex-1 min-h-0 flex flex-col lg:flex-row gap-2 lg:gap-3 overflow-hidden">
      <AppCard class="min-h-0 flex-1 min-w-0 flex flex-col" :padded="false">
        ...
        <header class="shrink-0 px-3 md:px-6 pt-3 md:pt-6 pb-2 md:pb-3 flex items-center gap-2 md:gap-3">
          <div class="relative shrink-0">
            <Transition name="hibi-fade">
              <div :key="format(selectedDate, 'yyyy-MM-dd')">
                <p class="hidden md:block text-[12.5px] text-fg-muted font-semibold capitalize">{{ format(selectedDate, t('journal.dateFormat.weekday'), { locale: dateLocale }) }}</p>
                <h2 class="text-[22px] md:text-[32px] font-extrabold text-fg leading-none tabular-nums">{{ format(selectedDate, 'd') }} <span class="text-[14px] md:text-[18px] text-fg-muted font-bold">{{ format(selectedDate, t('journal.dateFormat.month'), { locale: dateLocale }) }}</span></h2>
              </div>
            </Transition>
          </div>
          <div class="flex-1" />
          ...
          <button type="button" class="lg:hidden grid place-items-center size-10 rounded-full bg-muted text-fg-muted active:bg-inset shrink-0" :aria-label="t('journal.calendarEntriesAria')" @click="showSideMobile = true">
        ...
        <div class="hidden md:flex shrink-0 mx-6 mb-3 rounded-[12px] bg-sky-soft px-4 py-3 items-center gap-2.5">
          ...
          <div class="flex items-center gap-2 shrink-0">
            <button class="text-[12px] font-bold text-sky-deep hover:underline" @click="insertPrompt">{{ t('journal.prompt.use') }}</button>
            <button class="text-[12px] font-bold text-fg-muted hover:underline" @click="newPrompt">{{ t('journal.prompt.another') }}</button>
          </div>
        </div>
        ...
        <div class="flex-1 min-h-0 flex mx-3 md:mx-6 mt-1 mb-2 relative">
          <Transition name="hibi-fade">
            <AppRichEditor
              ...
              :placeholder="t('journal.editorPlaceholder')"
        ...
        <footer class="shrink-0 px-4 md:px-6 pb-4 md:pb-5 pt-1 flex items-center justify-end gap-2">
          <Transition name="hibi-fade">
            <span v-if="dirty" class="mr-auto text-[12px] text-fg-subtle font-semibold">{{ t('journal.saving') }}</span>
          </Transition>
          <AppButton v-if="selectedEntry" variant="ghost" size="sm" :aria-label="t('journal.deleteAria')" @click="removeSelected">
            <template #icon><Trash2 class="size-[15px]" :stroke-width="2.1" /></template>{{ t('journal.delete') }}
          </AppButton>
          <AppButton variant="primary" size="sm" @click="save">{{ t('journal.save') }}</AppButton>
        </footer>
      </AppCard>
      ...
      <aside class="flex-col"
        ...
        <div v-if="showSideMobile" class="lg:hidden shrink-0 flex items-center justify-between px-4 pb-2" style="padding-top: max(0.9rem, env(safe-area-inset-top))">
          <h3 class="text-[18px] font-extrabold text-fg">{{ t('journal.calendar') }}</h3>
          <button type="button" class="grid place-items-center size-9 rounded-full bg-muted text-fg-muted" :aria-label="t('common.close')" @click="showSideMobile = false">
        ...
        <AppCard class="!p-4 shrink-0">
          <div class="flex items-center justify-between mb-2">
            <button class="grid place-items-center size-7 rounded-[8px] text-fg-muted hover:bg-muted hover:text-fg" :aria-label="t('journal.prevMonth')" @click="cursor = subMonths(cursor, 1)"><ChevronLeft class="size-4" :stroke-width="2" /></button>
            <p class="text-[14px] font-bold text-fg capitalize">{{ format(cursor, t('journal.dateFormat.monthYear'), { locale: dateLocale }) }}</p>
            <button class="grid place-items-center size-7 rounded-[8px] text-fg-muted hover:bg-muted hover:text-fg" :aria-label="t('journal.nextMonth')" @click="cursor = addMonths(cursor, 1)"><ChevronRight class="size-4" :stroke-width="2" /></button>
          </div>
        ...
        <AppCard class="shrink-0 lg:flex-1 lg:min-h-0 flex flex-col" :padded="false">
          <h3 class="px-4 pt-4 pb-2 text-[13px] font-bold text-fg-muted shrink-0">{{ t('journal.entries') }}</h3>
          <div class="hibi-anim-slide-right lg:flex-1 lg:min-h-0 lg:overflow-y-auto lg:scroll-area px-2 pb-3 flex flex-col gap-1">
            ...
            <div v-else-if="!entries.length" class="px-3 py-8 flex flex-col items-center text-center gap-2">
              ...
              <p class="text-[13px] font-bold text-fg">{{ t('journal.empty.title') }}</p>
              <p class="text-[12px] text-fg-muted leading-snug">{{ t('journal.empty.subtitle') }}</p>
            </div>
            <!-- Entradas reales -->
            <button v-for="e in entries" v-else :key="e.id"
              ...
              <div class="flex-1 min-w-0">
                <p class="text-[13px] font-bold text-fg capitalize">{{ format(e.date, t('journal.dateFormat.weekdayDay'), { locale: dateLocale }) }}</p>
    ...
    <div v-else class="flex-1 min-h-0 flex flex-col md:flex-row gap-2 md:gap-3 overflow-y-auto md:overflow-hidden scroll-area">
      ...
      <AppCard class="!p-5 md:!p-6 w-full md:w-[420px] shrink-0 flex flex-col items-center gap-4 relative overflow-hidden">
        ...
        <div class="text-center relative z-10">
          <p class="text-[11.5px] font-bold text-fg-muted uppercase tracking-wide">{{ t('journal.stats.moodAverage') }}</p>
          ...
          <p class="text-[12.5px] text-fg-muted mt-1">{{ t('journal.stats.entries', { count: entries.length }) }}</p>
        </div>
        <!-- Donut radial interactivo -->
        <svg viewBox="0 0 240 240" class="w-full max-w-[260px] relative z-10" :aria-label="t('journal.stats.distributionAria')">
          <g transform="translate(120 120)">
            ...
            <text v-else text-anchor="middle" y="6" fill="var(--text-muted)" class="text-[13px] font-bold">{{ t('journal.stats.total') }}</text>
          </g>
        </svg>
      ...
      <AppCard class="!p-5 md:!p-6 flex-1 min-w-0 min-h-[300px] md:min-h-0 flex flex-col">
        <header class="flex items-center justify-between mb-4 shrink-0">
          <h3 class="text-[14px] font-bold text-fg-muted">{{ t('journal.stats.heatmapTitle') }}</h3>
          <p v-if="heatSelected" class="text-[12.5px] font-semibold text-fg">
            <span class="capitalize">{{ format(heatSelected.date, t('journal.dateFormat.heatSelected'), { locale: dateLocale }) }}</span>
            <span v-if="heatSelected.mood" class="ml-2" :style="{ color: moods[heatSelected.mood-1]!.hex }">· {{ moods[heatSelected.mood-1]!.label }}</span>
            <span v-else class="ml-2 text-fg-subtle">· {{ t('journal.stats.noEntry') }}</span>
          </p>
        </header>
          ...
          <div class="hibi-anim-bars flex-1 grid gap-1.5 min-h-0"
            ...
            <button v-for="(c, i) in heatCells" :key="i" type="button"
              ...
              :style="c.mood && (selectedMood === null || c.mood === selectedMood)
                ...
                : undefined"
              :title="`${format(c.date, t('journal.dateFormat.heatTitle'), { locale: dateLocale })}`"
        ...
        <div class="shrink-0 flex items-center justify-end gap-2 mt-4 pt-3 border-t border-[var(--bg-muted)]">
          <span class="text-[11px] text-fg-muted font-bold">{{ t('journal.stats.less') }}</span>
          <span v-for="m in moods" :key="m.v" class="size-3.5 rounded-[4px]" :style="{ background: m.hex, opacity: 0.4 + (m.v / 5) * 0.6 }"></span>
          <span class="text-[11px] text-fg-muted font-bold">{{ t('journal.stats.more') }}</span>
        </div>
      </AppCard>
    </div>
  </div>
